...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4296653,3655133)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4296653,3655133)
...
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (36306083,36304105,36304551,36306083,36304551,36304105)
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,-1,P.START_DATE) AND A.START_DATE <= DATEADD(day,7,P.START_DATE) AND A.visit_occurrence_id = P.visit_occurrence_id ) cc 
